 ...
 * @version $Id: jquery.activeform.js 3129 2011-03-26 16:30:16Z qiang.xue $
 * @since 1.1.1
 */
...
;(function($) {
	...
	$.fn.activeform = function(options) {
		return this.each(function() {
          ...
          disableAfterSubmit : settings.disableAfterSubmit, // SLS added
          disableCssClass : settings.disableCssClass, // SLS added
          disableText : settings.disableText // SLS added
				}, attribute);
      ...
      $form.find(':submit').live('mouseup keyup',function(){
        $form.data('submitObject',$(this));
          ...
          console.log(settings.disableAfterSubmit);
          var $button = $form.data('submitObject') || $form.find(':submit:last'); // SLS moved this here
          if (settings.disableAfterSubmit) {
            var $origText = $button.val();
            $button.val(settings.disableText)
                 .addClass(settings.disableCssClass)
                 .attr('disabled','disabled'); // SLS added
          }
					if(settings.beforeValidate==undefined || settings.beforeValidate($form)) {
						$.fn.activeform.validate($form, function(data){
							var hasError = false;
							$.each(settings.attributes, function(i, attribute){
								hasError = $.fn.activeform.updateInput(attribute, data, $form) || hasError;
							});
                  ...
                  // added support for the button name post submissions
                  /*var inputs = [];
                  ...
                  if (settings.disableAfterSubmit) $button.removeAttr('disabled'); // SLS added
									if ($button.length)
										$button.click();
                  else  // no submit button in the form
										$form.submit();
                  if (settings.disableAfterSubmit) $button.attr('disabled','disabled'); // SLS added
              ...
              if (settings.disableAfterSubmit) { // SLS added
                $button.val($origText)
                     .removeClass(settings.disableCssClass)
                     .removeAttr('disabled'); // SLS added
              }
						});
					}
					else {
			...
			} else { // regular validate (not onSubmit)
        if (settings.disableAfterSubmit) { // SLS added
          $form.submit(function(){
            settings.submitting=true;
            var $button = $form.data('submitObject') || $form.find(':submit:last'); // SLS moved this here
			...
			$form.bind('reset',function(){
				...
				setTimeout(function(){
					$.each(settings.attributes, function(i, attribute){
						...
						var $container = $.fn.activeform.getInputContainer(attribute, $form);
    ...
    disableAfterSubmit: false, // make the button unclickable on submit, to prevent duplicate submissions
