...
class TimeSheetSummaryController extends Controller
{
	...
	public function actionIndex()
	{
		...
		if(isset($_POST['Expenses'])){
			$where = " WHERE  t.id = ut.id_timesheet 
      and  ut.id_user = u.id
      and ut.id_task = pt.id
      and pt.id_project_phase = pp.id
      and p.id = pp.id_project
      and p.customer_id=c.id ";
			$i = 0;
			$select = "Select p.customer_id, p.id ,c.name , p.id_type ,p.name , u.firstname,u.lastname ,pt.description ,ut.comment  , ut.amount , ut.date  
   from projects_tasks pt, projects p , projects_phases pp ,user_time ut, users u,timesheets t , customers c";
			...
			if(isset($_POST['Expenses']['startDate'])&& $_POST['Expenses']['startDate']!=null && $_POST['Expenses']['endDate']!=null){
				$startDate = DateTime::createFromFormat('d/m/Y',$_POST['Expenses']['startDate'])->format('Y-m-d');
					...
					$where .= " AND ut.date between '$startDate' and '$endDate' ";
				$i=1;	
			}		
			$order = " ORDER BY p.name ASC";
				...
				{
					$resp[$row['customer_id']]['project'] = $row['id'];
					}
	...
	public function createExcel($resp){
		...
		->setCellValue('H5', $_POST['Expenses']['project_id'])
		...
		// Rename worksheet
		$objPHPExcel->getActiveSheet()->setTitle('Timesheets Reports');
		...
		$objPHPExcel->setActiveSheetIndex(0);
			...
			header('Content-Disposition: attachment;filename="Timesheets_Reports.xls"');
