<div class="mytabs hidden">
	...
	{
		$tabs = array(
			Yii::t('translations', 'General') => $this->renderPartial('_general_form_tab', array('model'=>$model), true),
		);
	}
	else
	{
			  	...
			  	if (data && data.status) {
				  	...
				  	if (data.status == "saved") {
					  	...
					  	if (data.update_connection) {
					  		$('#connections .tache.new').remove();
					  		$('#connections .new_conn').show();
					  		//$.fn.yiiGridView.update('connections-grid');
					  	}
					  	if (data.update_contact) {
					  		$('#contact_fields .tache.new').remove();
					  		$('#contact_fields .new_cont').show();
