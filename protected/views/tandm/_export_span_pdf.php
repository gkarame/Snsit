...
<table class="first">
	...
	<tr>
		<td class="h3" style="padding:5px;border-top:1.5px solid #567885;border-bottom:1.5px solid #567885;width:45%;"><b>Date:</b>  <?php echo (isset($model->invoice_date_month) && isset($model->invoice_date_year) ) ? date('d.m.Y',strtotime($model->invoice_date_year.'-'.$model->invoice_date_month.'-30')) : date('d.m.Y')?></td>
		...
		<td class="h3" style="padding:5px;border-top:1.5px solid #567885;border-bottom:1.5px solid #567885;text-align:right;width:50%;margin-left:20px"><b>Please pay before:</b><?php echo date('d.m.Y', strtotime($model->invoice_date_year.'-'.$model->invoice_date_month.'-30 +1 month' ))?></td>
	</tr>
	<tr>
		<td colspan="3"></td>
	</tr>
	...
	<tr>
		<td class="h3" style="padding:5px;">SPAN Trading | P.O. Box 55397 | Dubai, UAE.</td>
	...
	<tr>
		<td class="h3" style="padding:5px;">Tel# +971 4 2895011 | Fax # +971 4 2895022</td>
		<td style="padding:5px;border-bottom:1.5px solid #fff;width:29%"></td>
		<td class="h3" style="padding:5px;">Bank Name: Bank Of Beirut - Bauchrieh Branch </td>
	</tr>
	<tr>
		<td class="h3" style="padding:5px;">Attn: Mr.Tarek Saoud</td>
		<td style="padding:5px;border-bottom:1.5px solid #fff;width:29%"></td>
...
<table class="second">
	...
	<?php foreach ($models as $rez) { 
	 ...
	 if($rez->id_expenses == null)	{ ?>
		...
		<?php if(!empty($rez->idEa->expense) && $rez->idEa->expense != 'N/A' && $rez->idEa->expense != 'Actuals')
			{ ?>
				<tr style="text-align:center">
					<td style="text-align:center;border-right:1px solid #476976"><h3 style="font:15px Calibri;"><?php echo str_pad(++$i, 3, "0", STR_PAD_LEFT);?></h3></td>
					<td style="border-right:1px solid #476976"><h3 style="font:15px Calibri;"><?php echo Yii::t("translations", "Travel and Living Expenses");?></h3></td>
					<td style="text-align:center;border-right:1px solid #476976"><h3 style="font:15px Calibri;"><?php echo Codelkups::getCodelkup($rez->currency)?></h3></td>
					<td style="text-align:center;border-right:1px solid #476976"><h3 style="font:15px Calibri;"><?php echo Utils::formatNumber($rez->payment_procente/100*$rez->idEa->expense );
							$sum += $rez->payment_procente/100 * $rez->idEa->expense ;
	...
	<?php } ?>
	<tr class="th">
		<td colspan="2" style="border-top:1px solid #476976;font:14px Calibri;"><b><?php echo Utils::convert_number_to_words($sum);?></b></td>
	...
	<tr>
		<td colspan="4" class="h3" style="padding:2px;font-size:15px;;font-family:Calibri">As per invoice # <a href=<?php echo Yii::app()->createAbsoluteUrl('invoices/view',array('id'=>$model->id));?>><?php echo $model->partner_inv; ?></a>. This invoice represents <?php echo $model->sns_share?>% of the total SPAN Invoice.</td>
	</tr>
	<tr>
