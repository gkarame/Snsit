<div class="create supportCreate">	<?php $form=$this->beginWidget('CActiveForm', array('id'=>'support-desk-header-form',	'enableAjaxValidation'=>false,	'htmlOptions' => array(	'class' => 'ajax_submit',	'enctype' => 'multipart/form-data',	),	)); ?>
		<div class="row marginb20 marginr36 "><?php echo $form->labelEx($model,'short_description'); ?>
		...
		<?php if( $model->reason == 407 || $model->reason == 413) { ?>
			<div class="row marginb20 marginr36"><?php echo $form->labelEx($model, 'Type'); ?>
			<div class="selectBg_create"><?php echo $form->dropDownList($model, 'reason', array('407'=> 'WMS - Inquiry', '413' => 'General - Change Request') , array()); ?>
			</div>	<?php echo $form->error($model,'reason'); ?>	</div>	
		<?php }?>
		<div class="horizontalLine"></div>	<div class="row marginb20 marginr36"><?php echo $form->labelEx($model, 'product'); ?>
			...
			</div>	<?php echo $form->error($model,'severity'); ?>	</div>	<div class="row marginb20  row_textarea_ea">
			...
			</div>		<?php echo $form->error($model,'submitter_name'); ?> 	</div>

			

				<div class="row marginb20 marginr36 hidden br">
			...
			<?php if( $model->reason == 407 || $model->reason == 413) { ?>
			<div class="red" style="width:900px;"><i>Please enter a description of your inquiry. Kindly attach a screenshot where applicable. Remember, the more information you provide, the better our support team can help you.</i></div>
<?php }else{ ?>
			<div class="red" style="width:900px;"><i>Please enter a concise explanation of your issue. If you are reporting an error, kindly attach a screenshot of the error details window. Remember, the more information you provide, the better our support team can help you.</i></div>
<?php }?>
			<br/>
		 	<div class="Div2">	<?php echo $form->textArea($model,'description'); ?>	</div>		<?php echo $form->error($model,'description'); ?>	</div>	
		<div class="horizontalLine"></div>	
		<?php if( $model->reason != 407 && $model->reason != 413) { ?>
		<div class="row marginb20 marginr36  br right_bar">	<?php echo $form->labelEx($model, 'INCIDENT OCCURED PREVIOUSLY ? *'); ?>
			<div class="">	<?php echo $form->radioButtonList($model,'issue_incurred_previously',array('No'=>'No','Yes'=>'Yes'),array('checkValue'=>null)); ?>	</div>
			<?php echo $form->error($model,'issue_incurred_previously'); ?>	</div>	
			<div class="row marginb20 marginr36  br">
			...
			</div>	<?php echo $form->error($model,'issue'); ?>	</div>	
