<div class="mytabs hidden">
	...
	<?php $tabs = array();
	if (empty($model->id) && GroupPermissions::checkPermissions('customers-general_customers','write') && GroupPermissions::checknotCSonly()){
		$tabs = array(	Yii::t('translations', 'General') => $this->renderPartial('_general_form_tab', array('model'=>$model), true),);
	}else{
		if(GroupPermissions::checkPermissions('customers-general_customers','write') && GroupPermissions::checknotCSonly()){
			$tabs[Yii::t('translations', 'General')] = $this->renderPartial('_general_form_tab', array('model'=>$model), true);
		}else if (GroupPermissions::checknotCSonly()){
			$tabs[Yii::t('translations', 'General')] = $this->renderPartial('_general_tab', array('model'=>$model), true);
		}
		if(GroupPermissions::checkPermissions('customers-eas','write') && GroupPermissions::checknotCSonly()){
		...
		if(GroupPermissions::checkPermissions('customers-invoices','write') && GroupPermissions::checknotCSonly()){
		...
		if (GroupPermissions::checknotCSonly()){
...
<?php Yii::app()->session['limit'] = 3;	Yii::import("xupload.XUpload"); $x = new XUpload; $x->publishAssets(); ?>
		  	...
		  	success: function(data) {
			  	if (data && data.status) {
				  	...
				  	if (data.status == "saved") {
					  	...
					  	if (data.update_connection) {
					  		$('#connections .tache.new').remove();	$('#connections .new_conn').show();
					  	}
					  	if (data.update_contact) {
					  		$('#contact_fields .tache.new').remove();	$('#contact_fields .new_cont').show();
