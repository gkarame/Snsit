...
<div class="header_title">	<span class="red_title book"><?php echo Yii::t('translations', 'Trip Details');?></span></div>
	<div class="formColumn">	
		...
		<div class="row">
		<?php echo CHtml::activeLabelEx($model,'travel_supplier', array("style"=>"padding-top:7px;")); ?>			
			<div class="selectBg_create">
				<?php echo CHtml::activeDropDownList($model, 'travel_supplier', array(''=>'')); ?>
			</div>			
	...
	<div class="row">
		...
		<div class="row item inline-block normal ">
			...
			<div class="dataRow ">
			    ...
			    $this->widget('zii.widgets.jui.CJuiDatePicker',array(
			    	...
			    	'htmlOptions' => array(
			    		...
			    		'value'=> !isset($model->departure_date)? date("d/m/Y") : $model->departure_date,
			    	),
			...
			<?php echo CCustomHtml::error($model,'departure_date', array('id'=>"Booking_departure_date_em_")); ?>
		</div>
			...
			<div class="dataRow ">
			    ...
			    $this->widget('zii.widgets.jui.CJuiDatePicker',array(
			    	...
			    	'htmlOptions' => array(
			    		...
			    		'class' => 'datefield ',
			    		'style' => 'border: none;height:23px;',
			    		'value'=> !isset($model->return_date)? date("d/m/Y"):$model->return_date,
			    	),
	...
	<div class="row ">
		...
		<div class="row" >
			<?php echo CHtml::activeLabelEx($model, 'billable', array('style'=>'margin-bottom:15px;')); ?>
			
			 
				<?php echo $form->radioButtonList($model, 'billable', Booking::getRadioBut(),  array('separator' => "  ", 'labelOptions'=>array('style'=>'display:inline'))); ?>
...
<br/>	<div class="row">
			<?php echo CHtml::activeLabelEx($model, 'hotel_booking'); ?>
				...
				<?php echo $form->radioButtonList($model, 'hotel_booking', Booking::getRadioBut(),  array('separator' => "  ", 'labelOptions'=>array('style'=>'display:inline'))); ?>
		...
		<?php echo CHtml::activeLabelEx($model,'hotel_supplier'); ?>			
			<div class="selectBg_create">
				<?php echo CHtml::activeDropDownList($model, 'hotel_supplier',array(''=>''),  array('onchange'=>'refreshfields();')); ?>
			</div>			
		...
		<?php echo CHtml::activeLabelEx($model,'car_supplier'); ?>			
			<div class="selectBg_create">
				<?php echo CHtml::activeDropDownList($model, 'car_supplier',  array(''=>''), array('onchange'=>'refreshfieldsCar();')); ?>
			</div>			
...
<div class="row" >
			<?php echo CHtml::activeLabelEx($model, 'car_booking'); ?>
				...
				<?php echo $form->radioButtonList($model, 'car_booking', Booking::getRadioBut(),  array('separator' => "  ", 'labelOptions'=>array('style'=>'display:inline'))); ?>
	
		 
			
			<?php echo CCustomHtml::error($model,'car_booking', array('id'=>"Booking_car_booking_em_")); ?>
		</div>
<div class="row item inline-block hidden normal" style="margin-top:3px;">
		...
		<div class="row item inline-block hidden normal" style="margin-top:2px;">
...
<script> 
	...
	var testProjectActualsExpensesUrl = '<?php echo Yii::app()->createAbsoluteUrl('projects/getBillablity');?>';
...
$(function() {
		changeBooking();
		if($("#Booking_traveler").val() == '')
		{ 
			var user=<?php echo Yii::app()->user->id; ?>;
			$("#Booking_traveler").val(user);
			refreshbranch();
		}	
		refreshsupp('travel');
		refreshsupp('hotel');
		...
		refreshfieldsCar();
		refreshAlerts();
			$("#Booking_departure_time").val("Evening");
			$("#Booking_return_time").val("Evening");

	});
 $(document).on('click', '[name="Booking[hotel_booking]"]', function () {
		if($(this).val() == 'Yes'){
  ...
  $(document).on('click', '[name="Booking[car_booking]"]', function () {
		if($(this).val() == 'Yes'){
  	...
  	function refreshBillable(element){
		...
		$.ajax({
			...
			success: function(res){
				if (res.code == 200){			
					if (res.type == 'true')
					{	$('input[name="Booking[billable]"]').val(['Yes']);
					}else{
						$('input[name="Booking[billable]"]').val(['No']);
					}
				}
			}
		});
	}
  function refreshdate(element)
  {
  	var $datepicker = $('#Booking_checkin');
	$datepicker.datepicker();
	$datepicker.datepicker('setDate', element);
	var $datepicker2 = $('#Booking_pickup');
	$datepicker2.datepicker();
	$datepicker2.datepicker('setDate', element);
	refreshAlerts();
  }
  function refreshdate2(element)
  {
  	var $datepicker = $('#Booking_checkout');
	$datepicker.datepicker();
	$datepicker.datepicker('setDate', element);
	var $datepicker2 = $('#Booking_return');
	$datepicker2.datepicker();
	$datepicker2.datepicker('setDate', element);
	refreshAlerts(element);
  }
  function refreshbranch()
  {
  	...
  	$.ajax({type: "POST",data: {'traveler':user},	url: "<?php echo Yii::app()->createAbsoluteUrl('booking/getBranch')?>", dataType: "json",
		  	success: function(data) {
			  	if (data) {	
			  		if (data.status == 'success') {	$("#Booking_origin").val(data.branch); 	refreshsupp('travel');	} 
...
function refreshfieldsCar()
{
	...
	var radioValue = $('input[name="Booking[car_booking]"]:checked').val();
    if(radioValue == "Yes"  && hotel == 'Choose another supplier') 
	{
	...
	}else if(radioValue == "Yes" && radiohotel != "Yes") {
		$('#Booking_return').parents('.row').addClass('margint85');
	}else{
