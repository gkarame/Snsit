...
class GroupGridView extends CGridView {
    ...
    public $plusminus = '<span class="plus-minus" data-collapsed="1" href="javascript:void(0)"></span>';
    ...
    */
    private function renderExtraRow($row, $totals)
    {
        ...
        if($this->dataProvider->modelClass == 'SmaActions')
        {//houda
            $cont=explode('?', $content);
            $idrow=$cont[2];
            $solution=$cont[1];
            $content=$cont[0];
            echo '<tr class="project_thead" style="height:5px !important;">';echo '<td class="extrarow" colspan="'.$colspan.'">'.$this->plusminus.'</td></tr>';
            echo '<tr><td colspan="'.($colspan-1).'">'.$content.'</td><td onclick=\'showPop('.$idrow.');\'><a style="font-size:12px; font-style: italic;">Update Description</a></td>';
             echo '<tr><td colspan="'.($colspan-1).'"><br/>'.$solution.'</td><td onclick=\'showPopSol('.$idrow.');\'><a style="font-size:12px; font-style: italic;">Update Suggested Solution</a></td>';
            echo '</tr>';
            echo '<tr class="project_thead" style="height:5px !important;">';
            echo '</tr>';
        }else
        {
            echo '<tr class="project_thead">';
            echo '<td class="extrarow" colspan="'.$colspan.'">'.$this->plusminus.' '.$content.'</td>';
            echo '</tr>';
        }
