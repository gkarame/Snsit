<?php
class WidgetSoldByRevenues extends CWidget 
{
	public $widget;	
    ...
    public function run()
    {
    	$this->render('/widgets/soldByRevenues', array(
            'dis'=>$this,
        ));
    }
    public function CharChart($year = null)
    {
    	...
    	$data_chart = self::getSoldByRevenues($data); echo json_encode($data_chart);
    }	
    public static function getSoldByRevenues($date = '55',$date2 = '55',$date3 = '55')
    {
    	$results = Yii::app()->db->createCommand()
    				->select('invoices.id, invoices.currency, invoices.net_amount as amount, invoices.sold_by')
    				->from('invoices')
   		...
   		{	
   			...
   			if (!isset($sum[$result['sold_by']])) {	$sum[$result['sold_by']]['val'] = $amount;
   			} else {	$sum[$result['sold_by']]['val'] += $amount; }    		
   		}   		
   		...
   		{
   			if($cust[0]['id'] == 0)
   				array_push($data_chart,array('category' =>  'No set',
       		'value' => $cust['val']));
   			else	
