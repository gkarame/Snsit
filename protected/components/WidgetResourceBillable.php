<?php
class WidgetResourceBillable extends CWidget 
{
	public $widget;	
    ...
    public function run()
    {
    	$this->render('/widgets/resourceBillable', array(
            'dis'=>$this,
        ));
    }    
 	public function CharChart($year = null)
    {
    	Yii::app()->session['topbill'] ='';	Yii::app()->session['monthbill'] ='';
    	$month= date('Y-m-01',strtotime('now - 6 month'));	$current_date = date('Y-m-d',strtotime('now'));
		$values = array();	$sr = array();
		$values = Yii::app()->db->createCommand("select t1.id_user , TRUNCATE(((billable*100)/total) ,2) as nbperc from 
(
...
select uts.id , uts.id_user ,t.billable , sum(uts.amount) as amount  from users u , user_time uts , projects_tasks t where u.id=uts.id_user and u.active='1' and uts.id_task=t.id and uts.`default`=0 and t.billable IN ('Yes','No') and uts.status='1' and uts.date  between '$month' and '$current_date'  GROUP BY uts.id_user 
union all
select uts.id , uts.id_user ,dti.billable , sum(uts.amount) as amount  from users u , user_time uts , internal_tasks dti where u.id=uts.id_user and u.active='1' and uts.id_task=dti.id and uts.`default`=3 and dti.billable IN ('Yes','No') and uts.status='1' and uts.date  between '$month' and '$current_date'  GROUP BY uts.id_user 
union all
