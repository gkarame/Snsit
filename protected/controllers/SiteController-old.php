...
class SiteController extends Controller
{
	...
	{
		return array(
			'postOnly + delete + shareBy', // we only allow deletion via POST request
		);
	}
	...
	public function accessRules() {
		return array(
			array('allow',
				'actions'=>array('login', 'logout','changePassword', 'forgetPassword','ResetPassword'),
				'users'=>array('*'),
			),
			array('allow',
				'actions'=>array('changeLoginPicture', 'closeTab', 'GetWidget','rememberTab', 'autoComplete', 'shareBy', 'download'),
	...
	public function actionIndex()
	{
		...
		$this->render('index', array('dashboards' => Dashboards::getAllDashboards()));
	}
	...
	{  
		...
		{ 
			...
			//Yii::app()->db->createCommand("INSERT IGNORE INTO user_widgets (widget_id, user_id , `order`) VALUES ('".$_GET['id']."','".Yii::app()->user->id."','".$lastOrder['ord']+1."')")->execute();
			/*
			$widadd = new UserWidgets();
			$widadd->order = $lastOrder['ord']+1;
			$widadd->user_id = Yii::app()->user->id;
			$widadd->widget_id = $_GET['id'];
			$widadd->save();*/
		}
	...
	/**
	 * 
	 * View the dashboard with the id $id
	 * @param int $id
	 */
	...
	public function actionChangeLoginPicture()
	{
		...
		if (isset($_POST['submit'])) 
		{
			...
			if (!empty($model->upload_file) && $model->validate())  // check if uploaded file is set or not
			{
				//empty background picture directory
	...
	{	
		...
		if (isset($_GET['cioi2uef9'])  && $_GET['cioi2uef9']!=' ' && $_GET['cioi2uef9']!='' ){
			...
			Yii::app()->db->createCommand("UPDATE users SET password='".sha1($new_pass)."' WHERE id='".$id_user."'")->execute(); 
				$email = Yii::app()->db->createCommand("SELECT email FROM user_personal_details upd, users u WHERE u.id = upd.id_user and u.id ='".$id_user."' ")->queryScalar();
	...
	/**
	 * 
	 * Sets on session the last subtab clicked by the user for the current controller/action 
	 */
	...
	/**
	 * 
	 * Removes from session the tab represented by controller/action/id key
	 */
	...
	public function actionShareBy($id)
	{
		...
		if (isset($_POST['model']) && $id > 0)
		{
			...
			{
				case 'eas':
					$item = Eas::model()->findByPk($id);
					$this->generatePdf('eas', $id); 
					break;
				case 'invoices':
					$item = Invoices::model()->findByPk($id);
					break;
				case 'documents':
					$item = Documents::model()->findByPk($id);
					break;
				case 'invoicesshare':
					$item = Invoices::model()->findByPk($id);
					if ($item->final_invoice_number == null)
			...
			if ($item) 
			{
				...
				{	
					if ($class == 'invoicesshare' || $class == 'invoices')
					{
						if ($item->final_invoice_number != null)
						{
							$file = $item->getFilePrinted(true);
						}
						else
						{
							$file = $item->getFileShare(true);
						}
					}
					else 
					{
						$file = $item->getFile(true);
					}
					...
					if ($model->validate()  && $file != NULL) 
					{
						...
						$sent = Yii::app()->mailer->MsgHTML("<div style='font-size:11pt;font-family:Calibri;'>".nl2br($model->body)."</div>")
										->AddFile($file)
										->Send(true);
				...
				$form = $this->renderPartial('shareby', array('model'=> $model, 'item'=>$item, 'class'=>$class), true, true);
	        	echo json_encode(array(
					"status"=>"failure", 
					"form" => $form,
	...
	public function actionDownload($file) 
	{
		...
		/**/
	/* Display the public holidays  Ramy Khattar Public Holiday Screen
	/**/
