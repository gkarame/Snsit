...
$(document).ready(function() {
	...
	checkContainer1(); //SD - number of customers vs SRs Submitted
	checkContainer2(); //SD - issue resolution time
	checkContainer3(); //SD - Top 10 Customers
	checkContainer4(); //SD - SRs Submitted per Customer
	checkContainer9(); //SD - Number of SRs Closed
	checkContainer11(); //SD - Resource Support Allocation
	checkContainer13(); //SD - SRs Submitted per Month
	checkContainer14(); //SD - SRs Submitted by Reason
	checkContainer18(); //SD - SRs Open by Status
	checkContainer19(); //SD - SRs Open by Severity
	checkContainer17(); //SD - System Shut Down SRs
	checkContainer23(); //SD - SRs Closed By Resource
	checkContainer28(); //SD - Support Performance
	checkContainer29(); //SD - Customer satisfaction variation
...
{
	$("#confirm_dialog_not .confirm_title").html(title_msg);
	$("#confirm_dialog_not .confirm_content").html(output_msg);
	$("#confirm_dialog_not").dialog({
        resizable: false,
        draggable: true,
        closeOnEscape: true,
        ...
        width: 700,
        height: 600,
        dialogClass: 'confirm_dialog_not'
    });
}
...
var parent = $(element).parent(); 
	if (parent.find('u').is(':visible')) {
		...
		parent.find('.panel').css({'top':(-25-height)+'px'});
	}
}
function showToolTaskTip(element) {
	
	var parent = $(element).parent(); 
	if (parent.find('u').is(':visible')) {
		parent.find('.panel_expenses').show();
		height = parent.find('.cover').outerHeight();
		parent.find('.panel_expenses').css({'top':(-25-height)+'px'});
	}
}
function showToolNoteTip(element) {
	
	var parent = $(element).parent(); 
	if (parent.find('u').is(':visible')) {
		parent.find('.panelNotes').show();
		height = parent.find('.cover').outerHeight();
		parent.find('.panelNotes').css({'top':(-65-height)+'px'});
	}
}
...
function hideToolTaskTip(element) {
	$(element).parent().find('.panel_expenses').fadeOut(100);
}
function hideToolNoteTip(element) {
	$(element).parent().find('.panelNotes').fadeOut(100);
}
...
function mareste(id){
	...
	if($('#graph-billability1').is(':visible')){ //if the container is visible on the page
		if($(".status.month_billability.colorRed").data("id") == null)
		{
			createGridBillability();
		}else{
			changeBillability($(".status.month_billability.colorRed").data("id"));
		}
		if($(".status.resc_billability.colorRed").data("id") == null)
		{
			createGridBillability();
		}else{
			changeResources($(".status.resc_billability.colorRed").data("id"));
		}
	}
	...
	if($('#graph-resource-non-billable1').is(':visible')){ //if the container is visible on the page
		changeResourceNonBillable($(".status.resource_non_billable.colorRed").data("id"));
	}
	if($('#graph-resource-billable1').is(':visible')){ //if the container is visible on the page
		changeResourceBillable($(".status.resource_billable.colorRed").data("id"));
	}
	if($('#graph-most-active-projects1').is(':visible')){ //if the container is visible on the page
		changeMonthProjects($(".status.active_projects.colorRed").data("id"));
		changeTopProjects($(".status.top_projects.colorRed").data("id"));
	...
	if($('#graph-sr-submitted-cust1').is(':visible')){ //if the container is visible on the page
		...
		if($('#graph-sr-system-down1').is(':visible')){ //if the container is visible on the page
			createGridStackSystemDown();
	}
	if($('#graph-project-alerts1').is(':visible')){ //if the container is visible on the page
			changeProjectAlerts();
	}
	if($('#graph-sr-openseverity-customer1').is(':visible')){ //if the container is visible on the page
			createGridopenseverityCustomer();
	}
	if($('#graph-sr-openstatus-customer1').is(':visible')){ //if the container is visible on the page
			createGridOpenStatusCustomer();
	}
...
//new
		if($('#graph-actuals2').is(':visible')){ //if the container is visible on the page
			
				if($(".status.status_actuals.colorRed").data("id") == null && $(".status.type_actuals.colorRed").data("id") == null){					 
							 createGridActuals();  						  
					}else{


					 if($(".status.status_actuals.colorRed").data("id") ==0 || $(".status.status_actuals.colorRed").data("id") ==1 || $(".status.status_actuals.colorRed").data("id") ==2 || $(".status.status_actuals.colorRed").data("id") ==3){
						changeStatusActuals($(".status.status_actuals.colorRed").data("id"));
					}

					if($(".status.type_actuals.colorRed").data("id") ==26 || $(".status.status_actuals.colorRed").data("id") ==27 || $(".status.status_actuals.colorRed").data("id") ==28 ){
						changeStatusActuals($(".status.type_actuals.colorRed").data("id"));
					}
				...
				//ADDING
				if($('#graph-custSatisfaction2').is(':visible')){ //if the container is visible on the page
			
				if($(".status.status_custSatisfaction.colorRed").data("id") == null && $(".status.type_custSatisfaction.colorRed").data("id") == null){					 
							 createGridActuals();  						  
					}else{


					 if($(".status.status_custSatisfaction.colorRed").data("id") ==0 || $(".status.status_custSatisfaction.colorRed").data("id") ==1 || $(".status.status_custSatisfaction.colorRed").data("id") ==2 || $(".status.status_custSatisfaction.colorRed").data("id") ==3){
						changeStatusActuals($(".status.status_custSatisfaction.colorRed").data("id"));
					}

					if($(".status.type_custSatisfaction.colorRed").data("id") ==26 || $(".status.status_custSatisfaction.colorRed").data("id") ==27 || $(".status.status_actuals.colorRed").data("id") ==28 ){
						changeStatusActuals($(".status.type_custSatisfaction.colorRed").data("id"));
					}
				...
				if($('#graph-support-performance1').is(':visible')){ //if the container is visible on the page
			
				if($(".status.status_sp.colorRed").data("id") == null && $(".status.type_sp.colorRed").data("id") == null){					 
							 createGridSupportPerformance();  						  
					}else{


					 if($(".status.status_sp.colorRed").data("id") ==0 || $(".status.type_sp.colorRed").data("id") ==1 || $(".status.status_sp.colorRed").data("id") ==2 || $(".status.status_sp.colorRed").data("id") ==3){
						changeStatusSupportPerformance($(".status.status_sp.colorRed").data("id"));
					}

					if($(".status.status_sp.colorRed").data("id") ==26 || $(".status.status_sp.colorRed").data("id") ==27 || $(".status.status_sp.colorRed").data("id") ==28 ){
						changeStatusSupportPerformance($(".status.type_sp.colorRed").data("id"));
					}
...
function checkContainer1 () {
	  if($('#pieSubmittedCustomer').is(':visible')){ //if the container is visible on the page
	    ...
	    setTimeout(checkContainer1, 5); //wait 50 ms, then try again
	  }
	}
function checkContainer2 () {
	  if($('#pieChartContainerTime').is(':visible')){ //if the container is visible on the page
	    ...
	    setTimeout(checkContainer2, 5); //wait 50 ms, then try again
	  }
	}
function checkContainer3 () {
	  if($('#graph-sr-top-customer').is(':visible')){ //if the container is visible on the page
	    ...
	    setTimeout(checkContainer3, 5); //wait 50 ms, then try again
	  }
	}
function checkContainer4 () {
	  if($('#graph-sr-customer').is(':visible')){ //if the container is visible on the page
	    ...
	    setTimeout(checkContainer4, 5); //wait 50 ms, then try again
	  }
	}
function checkContainer5 () {
	  if($('#graph-soldBy-revenues').is(':visible')){ //if the container is visible on the page
	    ...
	    setTimeout(checkContainer5, 5); //wait 50 ms, then try again
	  }
	}
function checkContainer6 () {
	  if($('#graph-ea-revenues').is(':visible')){ //if the container is visible on the page
	    ...
	    setTimeout(checkContainer6, 5); //wait 50 ms, then try again
	  }
	}
function checkContainer7 () {
	  if($('#graph-country-revenues').is(':visible')){ //if the container is visible on the page
	    ...
	    setTimeout(checkContainer7, 5); //wait 50 ms, then try again
	  }
	}
function checkContainer8 () {
	  if($('#graphCustomer-1').is(':visible')){ //if the container is visible on the page
		  ...
		  setTimeout(checkContainer8, 5); //wait 50 ms, then try again
	  }
	}

	//new 
function checkContainer22 () {
	  if($('#graphTest-1').is(':visible')){ //if the container is visible on the page
		  createGridTest1();  //Adds a grid to the html
	  } else {
		  setTimeout(checkContainer22, 5); //wait 50 ms, then try again
	  }
	}
...
function checkContainer9 () {
	  if($('#graph-sr').is(':visible')){ //if the container is visible on the page
		  ...
		  setTimeout(checkContainer9, 5); //wait 50 ms, then try again
	  }
	}
function checkContainer10 () {
	  if($('#chartContainerMaintenance').is(':visible')){ //if the container is visible on the page
		  ...
		  setTimeout(checkContainer10, 5); //wait 50 ms, then try again
	  }
	}
function checkContainer11 () {
	  if($('#graph-support').is(':visible')){ //if the container is visible on the page
		  ...
		  setTimeout(checkContainer11, 5); //wait 50 ms, then try again
	  }
	}
function checkContainer12 () {
	  if($('#chartContainerLine').is(':visible')){ //if the container is visible on the page
		  ...
		  setTimeout(checkContainer12, 5); //wait 50 ms, then try again
	  }
	}
function checkContainer13 () {
	  if($('#graph-sr-submitted').is(':visible')){ //if the container is visible on the page
	    ...
	    setTimeout(checkContainer13, 5); //wait 50 ms, then try again
	  }
	}
function checkContainer14 () {
	  if($('#graph-sr-submitted-reason').is(':visible')){ //if the container is visible on the page
	    ...
	    setTimeout(checkContainer14, 5); //wait 50 ms, then try again
	  }
	}
function checkContainer15 () {
	  if($('#graph-1').is(':visible')){ //if the container is visible on the page
	    ...
	    setTimeout(checkContainer15, 5); //wait 50 ms, then try again
	  }
	}
function checkContainer16 () {
	if($('#graph-billability').is(':visible')){ //if the container is visible on the page
	  ...
	  setTimeout(checkContainer16, 5); //wait 50 ms, then try again
	}
}
function checkContainer17(){
	  if($('#graph-sr-system-down').is(':visible')){ //if the container is visible on the page
		  ...
		  setTimeout(checkContainer17, 5); //wait 50 ms, then try again
	  }
}
function checkContainer18(){
	  ...
	  if($('#graph-sr-openstatus-customer').is(':visible')){ //if the container is visible on the page
		  ...
		  setTimeout(checkContainer18, 5); //wait 50 ms, then try again
	  }
}
...
function checkContainer19(){
	  if($('#graph-sr-openseverity-customer').is(':visible')){ //if the container is visible on the page
		  ...
		  setTimeout(checkContainer19, 5); //wait 50 ms, then try again
	  }
}
...
function checkContainer20 () {
	  if($('#graph-submittedresolved').is(':visible')){ //if the container is visible on the page
		  ...
		  setTimeout(checkContainer20, 5); //wait 50 ms, then try again
	  }
}

function checkContainer21 () {
	  if($('#graph-actuals').is(':visible')){ //if the container is visible on the page
		  createGridActuals();  //Adds a grid to the html
	  } else {
		  setTimeout(checkContainer21, 5); //wait 50 ms, then try again
	  }
	}

function checkContainer23 () {
	  if($('#graph-sr-close-resource').is(':visible')){ //if the container is visible on the page
		  changeMonthCloseRes(3);  //Adds a grid to the html
	  } else {
		  setTimeout(checkContainer23, 5); //wait 50 ms, then try again
	  }
	}



function checkContainer24 () {
	  if($('#graph-project-alerts').is(':visible')){ //if the container is visible on the page
		  changeProjectAlerts();  //Adds a grid to the html
	  } else {
		  setTimeout(checkContainer24, 5); //wait 50 ms, then try again
	  }
	}

function checkContainer25 () {
	  if($('#graph-resource-non-billable').is(':visible')){ //if the container is visible on the page
		  CreateGridRescNonBill();  //Adds a grid to the html
	  } else {
		  setTimeout(checkContainer25, 5); //wait 50 ms, then try again
	  }
	}


function checkContainer26 () {
	  if($('#graph-resource-billable').is(':visible')){ //if the container is visible on the page
		  CreateGridRescBill();  //Adds a grid to the html
	  } else {
		  setTimeout(checkContainer26, 5); //wait 50 ms, then try again
	  }
	}

function checkContainer27 () {
	  if($('#graph-most-active-projects').is(':visible')){ //if the container is visible on the page
		  CreateGridMostActiveProjects();  //Adds a grid to the html
	  } else {
		  setTimeout(checkContainer27, 5); //wait 50 ms, then try again
	  }
	}	

function checkContainer28 () {
  if($('#graph-support-performance').is(':visible')){ //if the container is visible on the page
	  createGridSupportPerformance();  //Adds a grid to the html
	  } else {
		  setTimeout(checkContainer28, 5); //wait 50 ms, then try again
	  }
	}
	function checkContainer29 () {
	  if($('#graph-custSatisfaction').is(':visible')){ //if the container is visible on the page
		  createGridCustomerSatisfaction();  //Adds a grid to the html
	  } else {
		  setTimeout(checkContainer29, 5); //wait 50 ms, then try again
	  }
	}
...
$(document).ready(function () {
    ...
    $('.userData .cancel').click(function () {
      ...
      $('.userData .qcancel').click(function () {
        $('.userData .qadd').fadeIn();
        $('.qualAdd').hide();
    });
});
